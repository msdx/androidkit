/*
 * @(#)LruCache.java		       Project:androidkit
 * Date:2013-5-13
 *
...
/**
 * 代码取自support-v4.jar包中的android/support/v4/util/LruCache.java。
 * 
...
public class MemoryLruCache<K, V> {
	...
	public final V put(K key, V value) {
		...
		if (preValue != null) {
			entryRemoved(false, key, preValue, value);
		}
			...
			synchronized (this) {
				if (size < 0 || map.isEmpty() && size != 0) {
					throw new IllegalStateException(getClass().getName()
							+ ".sizeOf() is reporting inconsistent result");
				}
	...
	public final V remove(K key) {
		...
		V preValue;
		synchronized (this) {
			preValue = map.remove(key);
			if (preValue != null) {
				size -= safeSizeOf(key, preValue);
			}
		}

		if (preValue != null) {
			entryRemoved(false, key, preValue, null);
		}
		return preValue;
	}
